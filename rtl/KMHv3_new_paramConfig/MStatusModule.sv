...
module MStatusModule(
  ...
  reg        reg_MPV;
  reg        reg_MDT;
  wire       _sstatus_SD_output = (&reg_FS) | (&reg_VS);
  wire       _sstatus_SDT_output = reg_SDT & menvcfg_DTE;
  ...
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      if (robCommit_vsDirty | writeVCSR | robCommit_vstart_valid
          & (|robCommit_vstart_bits))
        reg_VS <= 2'h3;
      else if (_GEN_1)
        reg_VS <=
          (wAliasSstatus_wen ? wAliasSstatus_wdata[10:9] : 2'h0)
          | (w_wen ? w_wdata[10:9] : 2'h0);
      if (w_wen & (w_wdata[12:11] == 2'h0 | w_wdata[12:11] == 2'h1 | (&(w_wdata[12:11])))
          | _GEN)
        reg_MPP <=
  ...
  assign sstatus_SDT = _sstatus_SDT_output;
  assign sstatus_SD = _sstatus_SD_output;
  assign sstatusRdata =
    {_sstatus_SD_output,
     38'h100,
     _sstatus_SDT_output,
