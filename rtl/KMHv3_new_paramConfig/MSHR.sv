...
module MSHR(
  ...
  wire             _mp_grant_task_mp_grant_param_T_4 = req_param == 3'h2;
  ...
  wire             _GEN_8 = io_resps_sink_d_valid & _GEN_6;
  wire             _GEN_9 = io_resps_sink_d_valid & _GEN_5;
  wire             _GEN_10 = io_replResp_valid & io_replResp_bits_retry;
  wire             _GEN_11 = io_replResp_valid & ~io_replResp_bits_retry;
  wire             _GEN_12 = _GEN_11 & (|io_replResp_bits_meta_state);
  wire             _GEN_13 =
    _GEN_11 & (|io_replResp_bits_meta_state) & io_replResp_bits_meta_clients;
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      gotT <= _GEN_8 ? io_resps_sink_d_bits_param == 3'h0 : ~io_alloc_valid & gotT;
      gotDirty <=
        _GEN_8 ? gotDirty | io_resps_sink_d_bits_dirty : ~io_alloc_valid & gotDirty;
      gotGrantData <= _GEN_9 | ~io_alloc_valid & gotGrantData;
      ...
      corrupt <= _GEN_9 ? io_resps_sink_d_bits_corrupt : ~io_alloc_valid & corrupt;
      ...
      state_s_retry <= ~_GEN_10 & (_GEN_2 | io_alloc_valid | state_s_retry);
      ...
      state_w_releaseack <=
        ...
        & (io_resps_sink_d_valid & io_resps_sink_d_bits_opcode == 3'h6 | io_alloc_valid
           | state_w_releaseack);
      state_w_replResp <=
        _GEN_11 | (io_alloc_valid ? io_alloc_bits_state_w_replResp : state_w_replResp);
  ...
  `ifdef ENABLE_INITIAL_REG_
    ...
    initial begin
      ...
      `ifdef RANDOMIZE_REG_INIT
        ...
        req_reqSource = _RANDOM[5'h7][20:16];
        dirResult_hit = _RANDOM[5'hB][0];
        dirResult_tag = _RANDOM[5'hB][31:1];
        dirResult_set = _RANDOM[5'hC][8:0];
        dirResult_way = _RANDOM[5'hC][11:9];
        dirResult_meta_dirty = _RANDOM[5'hC][12];
        dirResult_meta_state = _RANDOM[5'hC][14:13];
        dirResult_meta_clients = _RANDOM[5'hC][15];
        dirResult_meta_alias = _RANDOM[5'hC][17:16];
        dirResult_meta_prefetch = _RANDOM[5'hC][18];
        dirResult_meta_prefetchSrc = _RANDOM[5'hC][22:19];
        dirResult_meta_accessed = _RANDOM[5'hC][23];
        dirResult_meta_tagErr = _RANDOM[5'hC][24];
        dirResult_meta_dataErr = _RANDOM[5'hC][25];
        state_s_acquire = _RANDOM[5'hD][7];
        state_s_rprobe = _RANDOM[5'hD][8];
        state_s_pprobe = _RANDOM[5'hD][9];
        state_s_release = _RANDOM[5'hD][10];
        state_s_probeack = _RANDOM[5'hD][11];
        state_s_refill = _RANDOM[5'hD][12];
        state_s_retry = _RANDOM[5'hD][13];
        state_s_cmoresp = _RANDOM[5'hD][14];
        state_s_cmometaw = _RANDOM[5'hD][15];
        state_w_rprobeackfirst = _RANDOM[5'hD][16];
        state_w_rprobeacklast = _RANDOM[5'hD][17];
        state_w_pprobeackfirst = _RANDOM[5'hD][18];
        state_w_pprobeacklast = _RANDOM[5'hD][19];
        state_w_grantfirst = _RANDOM[5'hD][20];
        state_w_grantlast = _RANDOM[5'hD][21];
        state_w_grant = _RANDOM[5'hD][22];
        state_w_releaseack = _RANDOM[5'hD][23];
        state_w_replResp = _RANDOM[5'hD][24];
        mergeA = _RANDOM[5'hD][25];
        mp_grant_task_merge_task_off = _RANDOM[5'hF][13:8];
        mp_grant_task_merge_task_alias = _RANDOM[5'hF][15:14];
        mp_grant_task_merge_task_vaddr = {_RANDOM[5'hF][31:16], _RANDOM[5'h10][27:0]};
        mp_grant_task_merge_task_isKeyword = _RANDOM[5'h10][28];
        mp_grant_task_merge_task_opcode = {_RANDOM[5'h10][31:29], _RANDOM[5'h11][0]};
        mp_grant_task_merge_task_param = _RANDOM[5'h11][3:1];
        mp_grant_task_merge_task_sourceId = _RANDOM[5'h11][13:7];
        io_msInfo_bits_blockRefill_REG = _RANDOM[5'h16][18];
        io_msInfo_bits_blockRefill_REG_1 = _RANDOM[5'h16][19];
        io_msInfo_bits_blockRefill_REG_2 = _RANDOM[5'h16][20];
      `endif // RANDOMIZE_REG_INIT
      if (reset) begin
        gotT = 1'h0;
        gotDirty = 1'h0;
  ...
  assign io_tasks_source_b_bits_param =
    state_s_pprobe
      ? (_mp_grant_task_mp_grant_useProbeData_T & _b_task_io_tasks_source_b_bits_param_T_2
           ? 2'h1
           : 2'h2)
      : req_param[1:0];
  ...
  assign io_tasks_mainpipe_bits_meta_state =
    mp_grant_valid
      ...
      : mp_release_valid | _mp_grant_task_mp_grant_param_T_4
          ? 2'h0
          : _mp_grant_task_mp_grant_param_T_6 ? 2'h1 : dirResult_meta_state;
  ...
  assign io_tasks_mainpipe_bits_mergeA = mp_grant_valid & mergeA;
  assign io_tasks_mainpipe_bits_aMergeTask_off =
    mp_grant_valid ? mp_grant_task_merge_task_off : 6'h0;
  assign io_tasks_mainpipe_bits_aMergeTask_alias =
    mp_grant_valid ? mp_grant_task_merge_task_alias : 2'h0;
  assign io_tasks_mainpipe_bits_aMergeTask_vaddr =
    mp_grant_valid ? mp_grant_task_merge_task_vaddr : 44'h0;
  assign io_tasks_mainpipe_bits_aMergeTask_isKeyword =
    mp_grant_valid & mp_grant_task_merge_task_isKeyword;
  ...
  assign io_tasks_mainpipe_bits_aMergeTask_sourceId =
    mp_grant_valid ? mp_grant_task_merge_task_sourceId : 7'h0;
