...
module WritebackEntry_15(
  ...
  wire         _GEN = io_mem_release_ready & busy;
  reg          r_counter;
  wire [1:0]   remain_clr =
    _GEN ? (remain_dup_1[0] ? 2'h1 : {remain_dup_1[1], 1'h0}) : 2'h0;
  reg          REG;
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      if (state == 2'h2 & io_mem_grant_valid)
        state <= 2'h0;
      else if (state == 2'h1 & (r_counter | ~req_hasData) & _GEN)
      ...
      if (_GEN)
        r_counter <= ~r_counter & req_hasData;
    end
