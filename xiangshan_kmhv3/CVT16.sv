...
module CVT16(
  ...
  wire [16:0] in =
    (is_fp2int
       ...
       : 17'h0) | (is_int2fp ? in_sext : 17'h0) | (is_vfr ? _in_T : 17'h0);
  reg         sign_reg0;
  wire        sign = (is_fp2int | is_int2fp) & in[16] | is_vfr & in[15];
  wire [4:0]  exp = (is_fp2int ? in[15:11] : 5'h0) | (is_vfr ? in[14:10] : 5'h0);
  wire [10:0] _sig_T_2 = is_fp2int ? in[10:0] : 11'h0;
  wire [9:0]  out_exp_clz_io_in = _sig_T_2[9:0] | (is_vfr ? in[9:0] : 10'h0);
  ...
  wire [16:0] in_abs =
    (is_int2fp & sign ? 17'(~in + 17'h1) : 17'h0) | (is_int2fp & ~sign ? in : 17'h0);
  ...
  wire        _out_sign_T =
    is_vfrsqrt7 & ~sign & (is_normal | ~(|exp) & (|out_exp_clz_io_in));
  ...
  wire [4:0]  _out_exp_normalized_T = 5'(5'h1D - exp_normalized);
    ...
    if (reset) begin
      ...
      if (io_fire) begin
        ...
        round_stickyIn_reg0 <=
          ...
          & (|(in_shift[3:0]));
        round_signIn_reg0 <= sign;
        is_normal_reg0 <= is_normal;
        is_inf_reg0 <= is_inf;
        is_nan_reg0 <= is_nan;
        is_neginf_reg0 <= sign & is_inf;
        is_neginf_negzero_reg0 <= sign & (is_normal | ~(|exp) & (|out_exp_clz_io_in));
        is_negzero_reg0 <= sign & ~(|exp) & out_exp_clz_io_in == 10'h0;
        is_poszero_reg0 <= ~sign & ~(|exp) & out_exp_clz_io_in == 10'h0;
        is_snan_reg0 <= ~(out_exp_clz_io_in[9]) & is_nan;
        is_neg2_bplus1_b_reg0 <= sign & _is_pos2_b_bplus1_T_1;
        is_neg2_b_bminus1_reg0 <= sign & _is_pos2_bminus1_b_T_1;
        is_neg2_negbminus1_negzero_reg0 <=
          sign & out_exp_clz_io_in[9:8] == 2'h0 & ~(|exp) & (|out_exp_clz_io_in);
        is_pos2_poszero_negbminus1_reg0 <=
          ~sign & out_exp_clz_io_in[9:8] == 2'h0 & ~(|exp) & (|out_exp_clz_io_in);
        is_pos2_bminus1_b_reg0 <= ~sign & _is_pos2_bminus1_b_T_1;
        is_pos2_b_bplus1_reg0 <= ~sign & _is_pos2_b_bplus1_T_1;
        in_orR_reg0 <= |in_sext;
        in_reg0 <= in;
        sign_reg0 <= sign;
        exp_of_reg0 <= is_fp2int & exp > max_int_exp;
        lpath_sig_shifted_reg0 <= _min_int_T ? _lpath_sig_shifted_T_2[15:0] : 16'h0;
        lpath_iv_reg0 <= _min_int_T & ~(io_opType[0]) & sign;
        ...
        result0_reg1 <=
          ...
          | (is_vfrsqrt7_reg0
             ...
             & ~(is_nan_reg0 | is_inf_reg0 | is_negzero_reg0 | is_poszero_reg0
               ...
               ? (is_vfrsqrt7_reg0
                    ? {out_sign_reg0, out_exp_reg0, sig_out7_reg0, 3'h0}
                    : 16'h0)
                 | (is_vfrec7_reg0
                      ? {sign_reg0,
                         out_exp_reg0,
    ...
    {11'h0,
     {_fflags0_T_46[4],
      _fflags0_T_46[3:0]
       ...
       | {_fflags0_T_23 & is_snan_reg0, 4'h0}} | {_GEN_3, 15'h0};
  always @(posedge clock) begin
    ...
    if (io_fire) begin
      rm_reg0 <= io_rm;
      lpath_of_reg0 <=
        _min_int_T & io_opType[0] & exp == max_int_exp & (~sign | sign & (|(in[9:0])));
      ...
      out_sign_reg0 <= _out_sign_T & sign;
    end
  end // always @(posedge)
