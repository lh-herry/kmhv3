...
module TrapTvalMod(
  ...
  input         io_fromCtrlBlock_flush_bits_backendIGPF,
  input         io_fromCtrlBlock_flush_bits_backendIPF,
  input         io_fromCtrlBlock_flush_bits_backendIAF,
  input         io_fromCtrlBlock_flush_bits_robIdx_flag,
  input  [8:0]  io_fromCtrlBlock_flush_bits_robIdx_value,
  input  [63:0] io_fromCtrlBlock_flush_bits_fullTarget,
  input         io_fromCtrlBlock_robDeqPtr_flag,
  input  [8:0]  io_fromCtrlBlock_robDeqPtr_value,
  ...
  reg  [63:0] tval;
  reg         robIdx_flag;
  reg  [8:0]  robIdx_value;
  wire        _clearFromFlush_T =
    io_fromCtrlBlock_flush_bits_backendIGPF | io_fromCtrlBlock_flush_bits_backendIPF;
  wire        updateFromFlush =
    io_fromCtrlBlock_flush_valid
    & (_clearFromFlush_T | io_fromCtrlBlock_flush_bits_backendIAF);
  wire        _GEN =
    io_targetPc_valid
  ...
  `ifdef ENABLE_INITIAL_REG_
    ...
    initial begin
      ...
      `ifdef RANDOMIZE_REG_INIT
        ...
        robIdx_value = _RANDOM[2'h2][10:2];
      `endif // RANDOMIZE_REG_INIT
      if (reset)
        valid = 1'h0;
    end // initial
