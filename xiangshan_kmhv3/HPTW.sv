...
module HPTW(
  ...
  wire [13:0]      _GEN_2 = {io_csr_hgatp_ppn[1:0], 12'h0};
  assign _GEN =
    (_GEN_1
  ...
  wire [47:0]      mem_addr =
    (_GEN_1 ? (&af_level) : af_level == 2'h2)
      ? {io_csr_hgatp_ppn[35:2],
         _GEN_1 ? _GEN_2 | {gpaddr[49:39], 3'h0} : _GEN_2 | {gpaddr[40:30], 3'h0}}
  ...
  wire             _GEN_5 = idle & (_GEN_4 | _GEN_3);
  wire             _GEN_6 = find_pte | accessFault;
  wire             _GEN_7 = io_resp_ready & resp_valid;
  assign finish = mem_addr_update & _GEN_6 & resp_valid;
  wire [1:0]       _GEN_8 =
    io_req_bits_l1Hit
      ? 2'h0
      : io_req_bits_l2Hit ? 2'h1 : _GEN_1 ? {1'h1, ~io_req_bits_l3Hit} : 2'h2;
  wire             _GEN_9 = sent_to_pmp & ~mem_addr_update;
  wire             _GEN_10 = accessFault & ~idle;
  wire             _GEN_11 = io_mem_req_ready & io_mem_req_valid_0;
  wire             _GEN_12 = io_mem_resp_valid & ~w_mem_resp;
  wire             _GEN_13 =
    bitmap_enable & (_find_pte_T | pte_perm_w) & pte_perm_v & ~pageFault;
  wire             _GEN_14 = io_bitmap_req_ready & ~s_bitmap_check;
  wire             _GEN_15 = ~w_bitmap_resp & io_bitmap_resp_valid;
  wire             _GEN_16 = mem_addr_update & ~_GEN_6;
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      if (_GEN_12)
        ...
        af_level <= _GEN_8;
      s_pmp_check <= flush | _GEN_10 | _GEN_9 | ~_GEN_4 & s_pmp_check;
      s_mem_req <= flush | ~_GEN_16 & (_GEN_11 | _GEN_10 | ~_GEN_9 & s_mem_req);
      w_mem_resp <= flush | _GEN_12 | ~_GEN_11 & (_GEN_10 | w_mem_resp);
      idle <= flush | mem_addr_update & _GEN_6 & _GEN_7 | ~_GEN_5 & idle;
      mem_addr_update <=
        ~(flush | mem_addr_update & (~_GEN_6 | _GEN_7))
        & (_GEN_15 | whether_need_bitmap_check & ~_GEN_13 | _GEN_12 & ~bitmap_enable
           | _GEN_10 | mem_addr_update);
      s_bitmap_check <=
        flush | _GEN_14 | ~(whether_need_bitmap_check & _GEN_13)
        & (_GEN_10 | ~_GEN_3 & s_bitmap_check);
      w_bitmap_resp <= flush | _GEN_15 | ~_GEN_14 & (_GEN_10 | w_bitmap_resp);
      whether_need_bitmap_check <=
        ~(flush | whether_need_bitmap_check)
        & (_GEN_12 ? bitmap_enable : ~_GEN_10 & whether_need_bitmap_check);
      bitmap_checkfailed <=
        ~flush & (~mem_addr_update | ~_GEN_6 | ~_GEN_7)
        & (_GEN_15 ? io_bitmap_resp_bits_cf : ~_GEN_10 & bitmap_checkfailed);
  ...
  wire [63:0]      _GEN_18 = _GEN_17[io_req_bits_gvpn[2:0]];
  wire             _GEN_19 =
  ...
  always @(posedge clock) begin
    ...
    if (_cache_level_T) begin
      ...
      cache_pte_n <=
        io_req_bits_bitmapCheck_fromSP ? io_req_bits_bitmapCheck_pte[63] : _GEN_18[63];
      cache_pte_pbmt <=
      ...
      cache_pte_ppn <=
        io_req_bits_bitmapCheck_fromSP
          ...
          : _GEN_18[45:10];
      cache_pte_perm_d <=
        io_req_bits_bitmapCheck_fromSP ? io_req_bits_bitmapCheck_pte[7] : _GEN_18[7];
      cache_pte_perm_a <=
        io_req_bits_bitmapCheck_fromSP ? io_req_bits_bitmapCheck_pte[6] : _GEN_18[6];
      cache_pte_perm_u <=
        io_req_bits_bitmapCheck_fromSP ? io_req_bits_bitmapCheck_pte[4] : _GEN_18[4];
      cache_pte_perm_x <=
        io_req_bits_bitmapCheck_fromSP ? io_req_bits_bitmapCheck_pte[3] : _GEN_18[3];
      cache_pte_perm_w <=
        io_req_bits_bitmapCheck_fromSP ? io_req_bits_bitmapCheck_pte[2] : _GEN_18[2];
      cache_pte_perm_r <=
        io_req_bits_bitmapCheck_fromSP ? io_req_bits_bitmapCheck_pte[1] : _GEN_18[1];
      cache_pte_perm_v <=
        io_req_bits_bitmapCheck_fromSP ? io_req_bits_bitmapCheck_pte[0] : _GEN_18[0];
    ...
    accessFault <=
      ~flush
      & (mem_addr_update & _GEN_6 & resp_valid
           ? ~(_GEN_7 | _GEN_4) & _GEN_19
           : ~_GEN_4 & _GEN_19);
    if (_GEN_5)
      id <= io_req_bits_id;
    if (way_info_REG)
