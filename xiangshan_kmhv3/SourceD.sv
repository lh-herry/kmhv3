...
module SourceD(
  ...
  wire         pipe_io_out_ready =
    _pipe_io_out_bits_needPb
      ...
      : s3_d_ready;
  assign s3_valid = pipe_io_out_ready & _pipe_io_out_valid;
  ...
  reg          beatsLeft;
  wire         winner_1 = ~s3_d_valid & s2_d_valid;
  reg          state_0;
  reg          state_1;
  wire         muxState_0 = beatsLeft ? state_0 : s3_d_valid;
  wire         muxState_1 = beatsLeft ? state_1 : winner_1;
  assign s3_d_ready = io_d_ready & (~beatsLeft | state_0);
  assign s2_d_ready = io_d_ready & (beatsLeft ? state_1 : ~s3_d_valid);
  wire         io_d_valid_0 =
    beatsLeft ? state_0 & s3_d_valid | state_1 & s2_d_valid : s3_d_valid | s2_d_valid;
  ...
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      s2_valid_pb <= s2_latch ? s1_need_pb : ~io_pb_pop_ready & s2_valid_pb;
      ...
      beatsLeft <=
        ~beatsLeft & io_d_ready
          ? s3_d_valid & _pipe_io_out_bits_req_opcode[0] & ~(_decode_T[5]) | winner_1
            & s2_req_opcode[0] & ~(_decode_T_3[5])
          : 1'(beatsLeft - (io_d_ready & io_d_valid_0));
      if (beatsLeft) begin
      end
      else begin
        state_0 <= s3_d_valid;
